...
def create_target_shapes(v):
    target_a = jnp.zeros((num_vertices, I))
    target_b = jnp.zeros((num_vertices, I))

    # Ellipse parameters
    a = 1.8  # Semi-major axis
    b = 0.3  # Semi-minor axis
    angle_a = 0.0  # Rotation angle for U1a ellipse (in radians)
    angle_b = jnp.pi / 2  # Rotation angle for U1b ellipse (perpendicular to U1a)
    ...
    # For U1a
    x_rot_a = x_p * jnp.cos(angle_a) + y_p * jnp.sin(angle_a)
    y_rot_a = -x_p * jnp.sin(angle_a) + y_p * jnp.cos(angle_a)
    ellipse_eq_a = (x_rot_a / a) ** 2 + (y_rot_a / b) ** 2

    # Create a mask for points inside the ellipse and in the northern hemisphere
    mask_a = (ellipse_eq_a <= 1) & (z_p >= 0)
    target_a = target_a.at[:, 0].set(jnp.where(mask_a, 1.0, 0.0))

    # For U1b
    x_rot_b = x_p * jnp.cos(angle_b) + y_p * jnp.sin(angle_b)
    y_rot_b = -x_p * jnp.sin(angle_b) + y_p * jnp.cos(angle_b)
    ellipse_eq_b = (x_rot_b / a) ** 2 + (y_rot_b / b) ** 2

    # Create a mask for points inside the ellipse and in the northern hemisphere
    mask_b = (ellipse_eq_b <= 1) & (z_p >= 0)
    target_b = target_b.at[:, 0].set(jnp.where(mask_b, 1.0, 0.0))
